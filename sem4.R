...
sem4.jg<-function(){
  ...
  for (i in 1:Nspec) {
    ...
    muRS[i] <- alphaRS+betaNL2*NL[i]+betaBM3*BM[i]
  }
...
params4 <- c("betaBM","betaBM2","betaNL","betaNL2","betaBM3")

sem4.mcmc<-jags(data=sem.data,model.file=sem4.jg,n.chains=3,n.iter=12000,n.burnin=2000,n.thin=10,parameters.to.save=params4)

samples.sem4 <- jags.samples(sem4.mcmc$model, 
                             c("WAIC","deviance"), 
                             type = "mean", 
                             n.iter = ni,
                             n.burnin = nb,
...
samples.sem4$p_waic <- samples.sem4$WAIC
samples.sem4$waic <- samples.sem4$deviance + samples.sem4$p_waic
tmp <- sapply(samples.sem4, sum)
waic.sem4 <- round(c(waic = tmp[["waic"]], p_waic = tmp[["p_waic"]]),1)
waic.sem4

save(file="sem4.Rdata", list=c("sem4.mcmc","samples.sem4","waic.sem4"))