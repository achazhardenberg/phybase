...
params6 <- c("betaBM","betaBM2","betaNL","betaBM3","betaRS")

sem6.mcmc<-jags.parallel(data=sem.data,model.file=sem6.jg,n.chains=3,n.iter=12000,n.burnin=2000,n.thin=10,parameters.to.save=params6)

samples.sem6 <- jags.samples(sem6.mcmc$model, 
                             c("WAIC","deviance"), 
                             type = "mean", 
                             n.iter = ni,
                             n.burnin = nb,
...
samples.sem6$p_waic <- samples.sem6$WAIC
samples.sem6$waic <- samples.sem6$deviance + samples.sem6$p_waic
tmp <- sapply(samples.sem6, sum)
waic.sem6 <- round(c(waic = tmp[["waic"]], p_waic = tmp[["p_waic"]]),1)
waic.sem6

save(file="sem6.Rdata", list=c("sem6.mcmc","samples.sem6","waic.sem6"))