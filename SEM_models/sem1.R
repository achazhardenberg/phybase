...
params1 <- c("betaBM","betaBM2","betaNL","betaDD")
sem1.mcmc<-jags(data=sem.data,model.file=sem1.jg,n.chains=3,n.iter=12000,n.burnin=2000, n.thin=10,parameters.to.save=params1)

samples.sem1 <- jags.samples(sem1.mcmc$model, 
                             c("WAIC","deviance"), 
                             type = "mean", 
                             n.iter = ni,
                             n.burnin = nb,
...
samples.sem1$p_waic <- samples.sem1$WAIC
samples.sem1$waic <- samples.sem1$deviance + samples.sem1$p_waic
tmp <- sapply(samples.sem1, sum)
waic.sem1 <- round(c(waic = tmp[["waic"]], p_waic = tmp[["p_waic"]]),1)
waic.sem1

save(file="sem1.Rdata", list="sem1.mcmc")
