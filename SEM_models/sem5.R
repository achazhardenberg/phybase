...
sem5.jg<-function(){
  ...
  for (i in 1:Nspec) {
    ...
    muRS[i] <- alphaRS+betaNL2*NL[i]+betaBM3*BM[i]+betaDD*DD[i]
  }
...
params5 <- c("betaBM","betaBM2","betaNL","betaNL2","betaBM3","betaDD")

sem5.mcmc<-jags.parallel(data=sem.data,model.file=sem5.jg,n.chains=3,n.iter=12000,n.burnin=2000,n.thin=10,parameters.to.save=params5)

samples.sem5 <- jags.samples(sem5.mcmc$model, 
                             c("WAIC","deviance"), 
                             type = "mean", 
                             n.iter = ni,
                             n.burnin = nb,
...
samples.sem5$p_waic <- samples.sem5$WAIC
samples.sem5$waic <- samples.sem5$deviance + samples.sem5$p_waic
tmp <- sapply(samples.sem5, sum)
waic.sem5 <- round(c(waic = tmp[["waic"]], p_waic = tmp[["p_waic"]]),1)
waic.sem5

save(file="sem5.Rdata", list=c("sem5.mcmc","samples.sem5","waic.sem5"))
