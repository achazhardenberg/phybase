...
bovidsCB2.mcmc<-jags(data=bovids.data, model.file=bovidsCB2.jg,n.chains=3,n.iter=24000,n.burnin=4000,n.thin=10,parameters.to.save=params)

samples.CB2 <- jags.samples(bovidsCB2.mcmc$model, 
                           c("WAIC","deviance"), 
                           type = "mean", 
                           n.iter = 24000,
                           n.burnin = 4000,
...
samples.CB2$p_waic <- samples.CB2$WAIC
samples.CB2$waic <- samples.CB2$deviance + samples.CB2$p_waic
tmp <- sapply(samples.CB2, sum)
waic.CB2 <- round(c(waic = tmp[["waic"]], p_waic = tmp[["p_waic"]]),1)

save(file="bovidsCB2.Rdata", list=c("bovidsCB2.mcmc", "samples.CB2","waic.CB2"))
